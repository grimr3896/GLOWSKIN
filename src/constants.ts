import { Product } from './types';

export const PRODUCTS: Product[] = [
  {
    "id": "cerave-hydrating-cleanser",
    "name": "CeraVe Hydrating Cleanser",
    "category": "skincare",
    "subcategory": "cleansers",
    "price": "$14.00",
    "image_url": "https://i.pinimg.com/736x/40/38/8b/40388be97d443b9b1b825026cabf5d35.jpg",
    "benefits": [
      "Gentle cleansing that removes dirt, oil, and makeup without stripping skin",
      "Restores and maintains the skin barrier with 3 essential ceramides (1, 3, 6-II)",
      "Hyaluronic acid attracts hydration and helps skin retain natural moisture",
      "MVE Technology provides 24-hour continuous hydration",
      "Non-comedogenic, fragrance-free, and paraben-free",
      "Suitable for normal to dry skin types",
      "National Eczema Association certified",
      "Leaves skin feeling hydrated, not tight or dry"
    ],
    "ingredients": "Water, Glycerin, Cetearyl Alcohol, PEG-40 Stearate, Stearyl Alcohol, Potassium Phosphate, Ceramide NP, Ceramide AP, Ceramide EOP, Carbomer, Glyceryl Stearate, Behentrimonium Methosulfate, Sodium Lauroyl Lactylate, Sodium Hyaluronate, Cholesterol, Phenoxyethanol, Disodium EDTA, Dipotassium Phosphate, Tocopherol, Phytosphingosine, Xanthan Gum, Cetyl Alcohol, Polysorbate 20, Ethylhexylglycerin",
    "how_to_use": [
      "Wet your face with lukewarm water",
      "Massage the cleanser onto your skin in gentle, circular motions",
      "Lather between your palms first if preferred",
      "Rinse thoroughly with lukewarm water",
      "Gently pat your skin dry with a clean towel",
      "Follow with your moisturizer and sunscreen",
      "Use twice daily, morning and night"
    ]
  },
  {
    "id": "cerave-foaming-cleanser",
    "name": "CeraVe Foaming Cleanser",
    "category": "skincare",
    "subcategory": "cleansers",
    "price": "$18.99",
    "image_url": "https://i.pinimg.com/1200x/b1/2d/23/b12d233ddacbe02ebe7b605d4c65fbaf.jpg",
    "benefits": [
      "Lightweight foaming formula perfect for oily and acne-prone skin",
      "Contains 3 essential ceramides to maintain skin barrier",
      "Niacinamide helps control oil production and reduces irritation",
      "Hyaluronic acid provides lightweight hydration without heaviness",
      "Removes excess oil, dirt, and makeup without over-drying",
      "Non-comedogenic and fragrance-free formula",
      "Oil-free and won't leave residue",
      "Suitable for normal to oily skin types"
    ],
    "ingredients": "Water, Glycerin, Sodium Lauroyl Lactylate, Ceramide NP, Ceramide AP, Ceramide EOP, Niacinamide, Sodium Hyaluronate, Cholesterol, Panthenol, Allantoin, Dimethicone, Citric Acid, Sodium Hydroxide, Sodium Benzoate, Xanthan Gum, Methylisothiazolinone",
    "how_to_use": [
      "Wet your face with lukewarm water",
      "Pump 1-2 times into your palms",
      "Massage onto your face in gentle, circular motions",
      "Work into a light foam",
      "Rinse thoroughly with lukewarm water",
      "Pat dry with a soft towel",
      "Use morning and night as part of your skincare routine",
      "Follow with toner and moisturizer appropriate for your skin type"
    ]
  },
  {
    "id": "cetaphil-gentle-cleanser",
    "name": "Cetaphil Gentle Cleanser",
    "category": "skincare",
    "subcategory": "cleansers",
    "price": "$13.99",
    "image_url": "https://i.pinimg.com/736x/87/51/61/8751611e9bd241e7f70369c735dee585.jpg",
    "benefits": [
      "Ultra-gentle, soap-free formula suitable for sensitive skin",
      "Removes makeup, dirt, and oil without disrupting skin barrier",
      "Hypoallergenic and fragrance-free",
      "Non-comedogenic - won't clog pores",
      "Dermatologist recommended for all skin types",
      "pH balanced to match natural skin pH",
      "Can be used on face and body",
      "Won't over-dry or strip the skin"
    ],
    "ingredients": "Water, Cetyl Palmitate, Stearyl Alcohol, Cetearyl Alcohol, Sodium Lauroyl Sarcosinate, Phenoxyethanol, Methylparaben, Propylparaben, Steareth-20, Citric Acid, Sodium Citrate, Disodium EDTA",
    "how_to_use": [
      "Dampen face with water or apply directly to dry skin",
      "Gently massage onto face and neck",
      "Rinse thoroughly with water",
      "Pat dry with a soft cloth",
      "Use 2-3 times daily or as needed",
      "For sensitive skin, use gentle circular motions",
      "Can also be used as a hand wash and body cleanser",
      "Follow with appropriate moisturizer"
    ]
  },
  {
    "id": "la-roche-posay-effaclar-cleanser",
    "name": "La Roche-Posay Effaclar Purifying Foaming Gel",
    "category": "skincare",
    "subcategory": "cleansers",
    "price": "$14.99",
    "image_url": "https://i.pinimg.com/736x/a0/1d/34/a01d34e3194c64b9dac120ab3dacb7e4.jpg",
    "benefits": [
      "Foaming gel specifically formulated for oily, acne-prone skin",
      "Contains salicylic acid to exfoliate and unclog pores",
      "Zinc PCA helps control sebum production",
      "Removes excess oil without over-drying the skin",
      "La Roche-Posay Thermal Spring Water soothes sensitive skin",
      "Reduces appearance of blemishes and blackheads",
      "Oil control that respects skin barrier",
      "Suitable for oily and combination skin types"
    ],
    "ingredients": "Water, Sodium Laureth Sulfate, Zinc PCA, Salicylic Acid, Lactic Acid, Glycerin, Sodium Chloride, Magnesium Sulfate, Potassium Sorbate, Sodium Benzoate, Disodium EDTA, Sodium Hydroxide, Phenoxyethanol",
    "how_to_use": [
      "Wet face with water",
      "Apply to face and massage gently for 30 seconds",
      "Work into a light lather",
      "Rinse thoroughly with lukewarm water",
      "Pat dry completely",
      "Use morning and evening",
      "For sensitive skin, start with once daily use",
      "Follow with La Roche-Posay toner and moisturizer"
    ]
  },
  {
    "id": "simple-refreshing-facial-wash",
    "name": "Simple Refreshing Facial Wash",
    "category": "skincare",
    "subcategory": "cleansers",
    "price": "$10.00",
    "image_url": "https://i.pinimg.com/736x/5b/bf/8e/5bbf8e44bd01217a4e1a313a4195b569.jpg",
    "benefits": [
      "Clean and simple formula with no harsh chemicals",
      "Removes makeup, dirt, and oil effectively",
      "Kind to sensitive skin despite gentle cleansing power",
      "Enriched with Vitamin B5 (Panthenol) to soothe",
      "Leaves skin feeling refreshed, not tight or dry",
      "Dermatologically tested on sensitive skin",
      "Free from perfume, alcohol, and parabens",
      "Suitable for all skin types, especially sensitive"
    ],
    "ingredients": "Water, Sodium Laureth Sulfate, Cocamidopropyl Betaine, Sodium Chloride, Panthenol, Glycerin, Sodium Benzoate, Citric Acid, Triethanolamine, Paraben-free formula",
    "how_to_use": [
      "Wet your face with water",
      "Apply a small amount of product",
      "Massage gently across face and neck",
      "Rinse thoroughly with lukewarm water",
      "Pat dry with a soft towel",
      "Use morning and evening",
      "For best results, follow with Simple moisturizer",
      "Works well for makeup removal when used with cotton pads"
    ]
  },
  {
    "id": "neutrogena-oil-free-acne-wash",
    "name": "Neutrogena Oil-Free Acne Wash",
    "category": "skincare",
    "subcategory": "cleansers",
    "price": "$10.99",
    "image_url": "https://i.pinimg.com/736x/71/75/a4/7175a4209f5f3853d5203a8e1140607f.jpg",
    "benefits": [
      "Contains 2% salicylic acid to treat and prevent acne",
      "Oil-free formula won't clog pores or cause breakouts",
      "Removes excess oil that contributes to acne",
      "Gentle enough for daily use",
      "Dermatologist recommended for acne-prone skin",
      "Budget-friendly and effective",
      "Helps clear blackheads and whiteheads",
      "Suitable for normal to oily skin types"
    ],
    "ingredients": "Water, Sodium Lauryl Sulfate, Salicylic Acid 2%, Cocamidopropyl Betaine, Sodium Chloride, Glycerin, Panthenol, Sodium Hydroxide, Disodium EDTA, Sodium Benzoate, Citric Acid",
    "how_to_use": [
      "Wet face with water",
      "Apply to affected areas or entire face",
      "Gently massage in circular motions",
      "Rinse thoroughly with water",
      "Pat dry with clean towel",
      "Use twice daily - morning and night",
      "For sensitive skin, start with once daily use",
      "Follow with oil-free moisturizer if needed"
    ]
  },
  {
    "id": "panoxyl-acne-foaming-wash",
    "name": "PanOxyl Acne Foaming Wash",
    "category": "skincare",
    "subcategory": "cleansers",
    "price": "$12.99",
    "image_url": "https://i.pinimg.com/736x/9b/32/35/9b3235987a223a5cc52474816a38c93f.jpg",
    "benefits": [
      "Contains benzoyl peroxide to kill acne-causing bacteria",
      "Foaming formula that cleanses without over-drying",
      "Effective against moderate acne and breakouts",
      "Available in 4%, 8%, and 10% strengths",
      "Oil-control formula for acne-prone skin",
      "Dermatologist recommended for stubborn acne",
      "Helps prevent future breakouts",
      "Works best when used consistently"
    ],
    "ingredients": "Water, Sodium C14-16 Olefin Sulfonate, Benzoyl Peroxide 4% or 10%, Cocamidopropyl Betaine, Glycerin, Sodium Chloride, Magnesium Sulfate, Disodium EDTA, Sodium Benzoate, Phenoxyethanol, Citric Acid",
    "how_to_use": [
      "Wet face with lukewarm water",
      "Apply a small amount to damp skin",
      "Massage gently onto affected areas",
      "Work into a light foam",
      "Rinse thoroughly with water",
      "Pat dry completely",
      "Use 1-2 times daily",
      "Start with lower concentration if new to benzoyl peroxide"
    ]
  },
  {
    "id": "cerave-moisturizing-cream",
    "name": "CeraVe Moisturizing Cream",
    "category": "skincare",
    "subcategory": "moisturizers",
    "price": "$23.99",
    "image_url": "https://i.pinimg.com/736x/2a/22/8b/2a228b3a0b8b3d8497b6154f9c26ffab.jpg",
    "benefits": [
      "Rich, non-greasy formula for very dry and sensitive skin",
      "Contains 3 essential ceramides to repair skin barrier",
      "Hyaluronic acid provides deep hydration",
      "MVE Technology delivers continuous moisturization",
      "Free from fragrance and dyes",
      "Non-comedogenic - won't clog pores",
      "Provides lasting 24-hour moisture",
      "Suitable for dry, eczema-prone, and sensitive skin"
    ],
    "ingredients": "Water, Glycerin, Petrolatum, Cetyl Alcohol, Ceramide NP, Ceramide AP, Ceramide EOP, Hyaluronic Acid, Cholesterol, Dimethicone, Phytosphingosine, Stearic Acid, Stearyl Alcohol, Polysorbate 60, Carbomer, Xanthan Gum, Disodium EDTA, Phenoxyethanol, Methylparaben, Propylparaben",
    "how_to_use": [
      "After cleansing, apply to slightly damp skin",
      "Use fingertips to gently massage in upward motions",
      "Allow product to fully absorb before applying makeup",
      "Use morning and evening",
      "Can be applied to face, neck, and body",
      "For very dry areas, use a bit more product",
      "Works well under sunscreen during the day",
      "Can be mixed with serums for enhanced hydration"
    ]
  },
  {
    "id": "neutrogena-hydro-boost",
    "name": "Neutrogena Hydro Boost Hyaluronic Acid Hydrating Gel-Cream",
    "category": "skincare",
    "subcategory": "moisturizers",
    "price": "$11.99",
    "image_url": "https://i.pinimg.com/736x/cf/f8/70/cff870c8a70014b6c85c26e41fe29524.jpg",
    "benefits": [
      "Lightweight gel-cream with hyaluronic acid for hydration",
      "Oil-free formula suitable for all skin types",
      "Absorbs quickly without greasy feel",
      "Provides 24-hour hydration",
      "Plumps and firms appearance of skin",
      "Non-comedogenic and fragrance-free",
      "Affordable and effective",
      "Great primer for makeup application"
    ],
    "ingredients": "Water, Glycerin, Hyaluronic Acid, Dimethicone, Isohexadecane, Polysorbate 80, Panthenol, Allantoin, Disodium EDTA, Sodium Hydroxide, Phenoxyethanol, Methylisothiazolinone",
    "how_to_use": [
      "Apply to clean, damp skin",
      "Massage gently until fully absorbed",
      "Use morning and evening",
      "Can be applied to face and neck",
      "Use sparingly - a little goes a long way",
      "For best results, apply to slightly damp skin",
      "Can layer with other products",
      "Wait 1-2 minutes before applying makeup or sunscreen"
    ]
  },
  {
    "id": "cetaphil-moisturizing-lotion",
    "name": "Cetaphil Moisturizing Lotion",
    "category": "skincare",
    "subcategory": "moisturizers",
    "price": "$12.99",
    "image_url": "https://i.pinimg.com/1200x/1c/27/55/1c2755d695bbe90273f47128dd741ee6.jpg",
    "benefits": [
      "Lightweight, non-greasy moisturizer for all skin types",
      "Hypoallergenic and fragrance-free",
      "Dermatologist recommended for sensitive skin",
      "Rich in glycerin for essential hydration",
      "Non-comedogenic - won't cause breakouts",
      "Fast-absorbing formula",
      "Suitable for use on face and body",
      "Can be used daily without irritation"
    ],
    "ingredients": "Water, Cetyl Palmitate, Glycerin, Stearyl Alcohol, Steareth-20, Panthenol, Sodium Hydroxide, Disodium EDTA, Methylparaben, Propylparaben",
    "how_to_use": [
      "Apply to clean skin morning and evening",
      "Use smooth, upward strokes",
      "Massage gently until fully absorbed",
      "Can be used on face, neck, and body",
      "Use more product for dry areas",
      "Works well under makeup and sunscreen",
      "No rinsing required",
      "Can be applied to damp or dry skin"
    ]
  },
  {
    "id": "aveeno-daily-moisturizing",
    "name": "Aveeno Daily Moisturizing Lotion",
    "category": "skincare",
    "subcategory": "moisturizers",
    "price": "$10.99",
    "image_url": "https://i.pinimg.com/736x/96/58/46/965846621c59e74844fc9a321aa114ad.jpg",
    "benefits": [
      "Lightweight, fast-absorbing lotion with oat extract",
      "Fragrance-free and hypoallergenic",
      "Contains natural oat extract to soothe skin",
      "Moisturizes for up to 24 hours",
      "Non-comedogenic and fragrance-free",
      "Dermatologist recommended for sensitive skin",
      "Affordable and effective daily moisturizer",
      "Suitable for all skin types including sensitive"
    ],
    "ingredients": "Water, Glycerin, Dimethicone, Cetyl Alcohol, Isopropyl Palmitate, Stearic Acid, Stearyl Alcohol, Polysorbate 60, Steareth-20, Avenaceous Sativa (Oat) Kernel Flour, Methylparaben, Propylparaben, Xanthan Gum, Sodium Chloride, Disodium EDTA, Cetyl Esters, Sodium Hydroxide",
    "how_to_use": [
      "Apply to clean skin twice daily",
      "Use on face, neck, and body as needed",
      "Massage gently into skin",
      "Absorbs quickly without greasy feel",
      "Can be applied to damp or dry skin",
      "Use more on very dry areas",
      "Safe for everyday use",
      "Works well under sunscreen and makeup"
    ]
  },
  {
    "id": "embryolisse-lait-creme",
    "name": "Embryolisse Lait-Crème Concentré",
    "category": "skincare",
    "subcategory": "moisturizers",
    "price": "$28.99",
    "image_url": "https://i.pinimg.com/736x/2a/f5/6f/2af56f479603e04d4a5b40d897eafae3.jpg",
    "benefits": [
      "Rich, creamy moisturizer with balancing formula",
      "Works as moisturizer, makeup remover, and massage cream",
      "Contains beeswax and mineral oil for nourishment",
      "Hydrates without leaving greasy residue",
      "Suitable for all skin types including combination",
      "French cult beauty product beloved by makeup artists",
      "Anti-aging properties with nourishing ingredients",
      "Can be used day or night, face or body"
    ],
    "ingredients": "Water, Mineral Oil, Glycerin, Beeswax, Cetyl Alcohol, Stearic Acid, Triethanolamine, Magnesium Sulfate, Borax, Sodium Benzoate, Potassium Sorbate, Parfum, Tocopherol",
    "how_to_use": [
      "Apply to clean, damp skin",
      "Use morning and evening",
      "Massage gently until absorbed",
      "Can use as makeup remover - apply and rinse",
      "Use small amount - cream is concentrated",
      "Works well under makeup",
      "Can be applied to neck and décolletage",
      "Allow to dry before applying other products"
    ]
  },
  {
    "id": "la-roche-posay-toleriane-moisturizer",
    "name": "La Roche-Posay Toleriane Double Repair Face Moisturizer",
    "category": "skincare",
    "subcategory": "moisturizers",
    "price": "$18.99",
    "image_url": "https://i.pinimg.com/736x/87/b2/3e/87b23e48952276b14613cdbaed058d38.jpg",
    "benefits": [
      "Lightweight, oil-free moisturizer for dry, sensitive skin",
      "Contains niacinamide to strengthen skin barrier",
      "Prebiotic Thermal Water soothes and protects",
      "Repairs damaged skin barrier",
      "24-hour hydration without greasy feel",
      "Fragrance-free and hypoallergenic",
      "Non-comedogenic formula",
      "Dermatologist recommended for dry and sensitive skin"
    ],
    "ingredients": "Water, Glycerin, Niacinamide, Dimethicone, Cetyl Alcohol, Distearyldimonium Chloride, Stearyl Alcohol, Isopropyl Isostearate, Sodium Lauroyl Lactylate, Xanthan Gum, Sodium Hydroxide, Disodium EDTA, Sodium Benzoate, Phenoxyethanol, Methylisothiazolinone",
    "how_to_use": [
      "Apply to clean, damp skin",
      "Use morning and evening",
      "Massage gently with fingertips",
      "Allow to fully absorb before makeup",
      "Can be applied to face and neck",
      "Use appropriate amount for your skin needs",
      "Works well layered with serums",
      "Safe for sensitive and reactive skin"
    ]
  },
  {
    "id": "olay-regenerist-micro-sculpting",
    "name": "Olay Regenerist Micro-Sculpting Cream",
    "category": "skincare",
    "subcategory": "moisturizers",
    "price": "$16.99",
    "image_url": "https://i.pinimg.com/736x/7b/13/5c/7b135c74b418836275980e92d7287fcf.jpg",
    "benefits": [
      "Anti-aging moisturizer with micro-lifting technology",
      "Contains adenosine to reduce fine lines",
      "Rich formula for mature or dry skin",
      "Firms and sculpts appearance of skin",
      "Visibly reduces wrinkles in 4 weeks",
      "Non-comedogenic and fragrance-free",
      "Dermatologist tested formula",
      "Provides deep hydration and nourishment"
    ],
    "ingredients": "Water, Glycerin, Dimethicone, Cetyl Alcohol, Stearic Acid, Stearyl Alcohol, Isohexadecane, Polysorbate 60, Steareth-20, Adenosine, Magnesium Sulfate, Disodium EDTA, Methylparaben, Propylparaben, Xanthan Gum, Sodium Benzoate, Phenoxyethanol",
    "how_to_use": [
      "Apply to clean skin morning and evening",
      "Use upward strokes to massage into skin",
      "Focus on areas with fine lines",
      "Allow to absorb completely",
      "Can be layered with serums",
      "Use appropriate amount for full coverage",
      "Works well under sunscreen",
      "Consistent use for best anti-aging results"
    ]
  },
  {
    "id": "neutrogena-ultra-sheer",
    "name": "Neutrogena Ultra Sheer Dry-Touch Sunscreen",
    "category": "skincare",
    "subcategory": "sunscreen",
    "price": "$12.99",
    "image_url": "https://i.pinimg.com/736x/25/b8/d4/25b8d4a38175439b437aad4a99315133.jpg",
    "benefits": [
      "Broad spectrum SPF 70+ protection against UVA/UVB rays",
      "Ultra-light, dry-touch formula doesn't feel greasy",
      "Oil-free and non-comedogenic",
      "Fast absorbing - won't clog pores",
      "Suitable for face and body",
      "Won't leave white cast on skin",
      "Affordable and widely available",
      "Can be worn under makeup"
    ],
    "ingredients": "Water, Avobenzone 3%, Octinoxate 7.5%, Homosalate 9%, Octisalate 5%, Oxybenzone 6%, Alcohol Denat, Polysorbate 80, Stearic Acid, Magnesium Sulfate, Fragrance, Sodium Benzoate, Phenoxyethanol, Xanthan Gum, Disodium EDTA",
    "how_to_use": [
      "Apply generously to all exposed skin",
      "Use 15 minutes before sun exposure",
      "Apply 1 ounce (shot glass full) to face and body",
      "Reapply every 2 hours or after swimming",
      "Can apply under or over moisturizer",
      "Use on all areas including ears and neck",
      "Smooth evenly without rubbing",
      "Safe for daily use"
    ]
  },
  {
    "id": "la-roche-posay-anthelios",
    "name": "La Roche-Posay Anthelios Ultra-Light Mineral Sunscreen",
    "category": "skincare",
    "subcategory": "sunscreen",
    "price": "$22.99",
    "image_url": "https://i.pinimg.com/1200x/72/80/a4/7280a4c0bffc17e17213344268a40380.jpg",
    "benefits": [
      "Mineral sunscreen with titanium dioxide and zinc oxide",
      "Broad spectrum SPF 60+ protection",
      "Water-resistant for 80 minutes",
      "Lightweight, non-greasy formula",
      "Prebiotic Thermal Spring Water soothes skin",
      "Non-comedogenic and fragrance-free",
      "Suitable for sensitive skin",
      "No chemical UV absorbers"
    ],
    "ingredients": "Water, Titanium Dioxide 15%, Zinc Oxide 9%, Isopropyl Stearate, Cetyl Alcohol, Dimethicone, Aluminum Hydroxide, Stearic Acid, Octyldodecyl Neopentanoate, Disodium EDTA, Phenoxyethanol, Sodium Hydroxide, Xanthan Gum",
    "how_to_use": [
      "Apply liberally 15 minutes before sun exposure",
      "Use 1/4 teaspoon for face and neck",
      "Reapply every 2 hours",
      "Reapply after swimming or sweating",
      "Works well under makeup",
      "Can be applied to sensitive areas",
      "Massage gently to avoid pilling",
      "Use daily for complete protection"
    ]
  },
  {
    "id": "supergoop-unseen-sunscreen",
    "name": "Supergoop Unseen Sunscreen SPF 40",
    "category": "skincare",
    "subcategory": "sunscreen",
    "price": "$42.00",
    "image_url": "https://i.pinimg.com/736x/68/f7/7f/68f77f662835768b515d45a5064032d8.jpg",
    "benefits": [
      "Invisible, weightless sunscreen that doesn't leave white cast",
      "SPF 40 broad spectrum protection",
      "Works under makeup and over moisturizers",
      "Silky, matte finish suitable for all skin types",
      "Oil-free formula won't clog pores",
      "Reapplyable throughout the day",
      "Dermatologist recommended",
      "Perfect for daily face protection"
    ],
    "ingredients": "Water, Homosalate 10%, Octinoxate 7.5%, Avobenzone 3%, Glycerin, Butylene Glycol, Pentylene Glycol, Propylene Glycol, Phenoxyethanol, Triethanolamine, Xanthan Gum, Disodium EDTA, Sodium Benzoate, Methylisothiazolinone",
    "how_to_use": [
      "Apply generously to face and neck daily",
      "Use as final step before makeup",
      "Reapply every 2 hours for extended sun exposure",
      "Can reapply over makeup without disruption",
      "Apply to all exposed areas including ears",
      "Use 15 minutes before sun exposure",
      "Works well with all skin types",
      "Can be used on lips and hands"
    ]
  },
  {
    "id": "biore-uv-aqua-rich",
    "name": "Biore UV Aqua Rich Watery Essence",
    "category": "skincare",
    "subcategory": "sunscreen",
    "price": "$11.99",
    "image_url": "https://i.pinimg.com/736x/71/95/c4/7195c4eea780377333f76cd00ee862fc.jpg",
    "benefits": [
      "Japanese sunscreen with watery, aqueous formula",
      "SPF 50+ PA++++ maximum protection",
      "Ultra-lightweight and water-based texture",
      "Won't leave sticky residue or white cast",
      "Suitable for all skin types including oily",
      "Quick-absorbing for daily use",
      "Can be used under makeup without pilling",
      "Provides long-lasting sun protection"
    ],
    "ingredients": "Water, Diethylamino Hydroxybenzoyl Hexyl Benzoate, Alcohol, Glycerin, Dimethicone, BG, Methyl Methacrylate, Sodium Chloride, Disodium EDTA, Sodium Hydroxide, Phenoxyethanol",
    "how_to_use": [
      "Apply to clean skin after toner and serums",
      "Use 1/4 teaspoon for face",
      "Extend to neck and ears",
      "Allow to set for 1 minute before makeup",
      "Reapply every 2-3 hours",
      "Can reapply with powder for convenience",
      "Use daily for comprehensive protection",
      "Works well with all skin types"
    ]
  },
  {
    "id": "black-girl-sunscreen-spf30",
    "name": "Black Girl Sunscreen SPF 30",
    "category": "skincare",
    "subcategory": "sunscreen",
    "price": "$28.99",
    "image_url": "https://i.pinimg.com/1200x/7e/22/d9/7e22d9faeede7206e134d62f4560fa0a.jpg",
    "benefits": [
      "Mineral sunscreen formulated for deeper skin tones",
      "Doesn't leave white cast on melanin-rich skin",
      "Broad spectrum SPF 30 protection",
      "Non-comedogenic formula",
      "Contains nourishing moisturizers",
      "Water-resistant for active lifestyles",
      "Cruelty-free and vegan option available",
      "Specifically developed for Black skin health"
    ],
    "ingredients": "Water, Zinc Oxide 18%, Titanium Dioxide 5%, Glycerin, Cetyl Alcohol, Stearyl Alcohol, Dimethicone, Beeswax, Cetearyl Alcohol, Stearic Acid, Sodium Stearoyl Glutamate, Phenoxyethanol, Methylparaben, Propylparaben",
    "how_to_use": [
      "Apply to clean, moisturized skin",
      "Use 1/4 teaspoon for face",
      "Apply 15 minutes before sun exposure",
      "Blend in thoroughly to avoid white cast",
      "Reapply every 2 hours",
      "Use on body and face",
      "Works well over makeup with powder application",
      "Daily sun protection is essential year-round"
    ]
  },
  {
    "id": "nivea-sun-protect-moisture",
    "name": "Nivea Sun Protect & Moisture Sunscreen",
    "category": "skincare",
    "subcategory": "sunscreen",
    "price": "$11.99",
    "image_url": "https://i.pinimg.com/736x/10/95/3c/10953c81de9e3ad1e8baed8a79372e94.jpg",
    "benefits": [
      "Combines sun protection with moisturizing benefits",
      "SPF 50+ broad spectrum protection",
      "Water-resistant for swimming",
      "Contains glycerin for hydration",
      "Non-comedogenic formula",
      "Suitable for all skin types",
      "Affordable and readily available",
      "Provides lasting protection without reapplication every hour"
    ],
    "ingredients": "Water, Glycerin, Ethylhexyl Methoxycinnamate, Butyl Methoxydibenzoylmethane, Titanium Dioxide, Cetyl Alcohol, Stearic Acid, Stearyl Alcohol, Glyceryl Stearate, Sodium Cetearyl Sulfate, Phenoxyethanol, Methylparaben, Propylparaben, Xanthan Gum, Sodium Chloride, EDTA",
    "how_to_use": [
      "Apply generously to all exposed areas",
      "Use 1/4 teaspoon for face",
      "Apply 15 minutes before sun exposure",
      "Reapply every 2 hours",
      "Reapply immediately after swimming",
      "Can be used as moisturizer and sunscreen combo",
      "Safe for daily use on face and body",
      "Apply to ears, neck, and top of feet"
    ]
  },
  {
    "id": "the-ordinary-niacinamide",
    "name": "The Ordinary Niacinamide 10% + Zinc 1%",
    "category": "skincare",
    "subcategory": "serums",
    "price": "$10.99",
    "image_url": "https://i.pinimg.com/736x/10/e9/09/10e909c799aec29b8056e8462e0b5ffc.jpg",
    "benefits": [
      "High-strength vitamin and mineral blemish formula",
      "Reduces appearance of skin blemishes and congestion",
      "Balances visible sebum activity",
      "Improves skin brightness",
      "Smooths skin texture",
      "Non-comedogenic and vegan",
      "Fragrance-free and alcohol-free",
      "Budget-friendly targeted treatment"
    ],
    "ingredients": "Water, Niacinamide, Pentylene Glycol, Zinc PCA, Dimethyl Isosorbide, Tamarix Chinensis Flower/Leaf/Stem Extract, Ethoxydiglycol, Phenoxyethanol, Chlorphenesin, Isoceteth-20, Ethylene Brassylate, Acacia Senegal Gum, Xanthan Gum",
    "how_to_use": [
      "Apply to entire face morning and evening",
      "Use before heavier creams",
      "Massage a few drops into skin",
      "If irritation occurs, cease use and consult a physician",
      "Do not use in the same routine as pure Vitamin C (L-Ascorbic Acid)",
      "Patch test before use is advised",
      "Keep out of reach of children",
      "Store in a cool, dry place"
    ]
  },
  {
    "id": "hyals-hydrating-serum",
    "name": "Hyals Hydrating Serum with Hyaluronic Acid",
    "category": "skincare",
    "subcategory": "serums",
    "price": "$16.99",
    "image_url": "https://i.pinimg.com/1200x/d7/98/b4/d798b4d68a0c6b7d53234ae70b80d1e7.jpg",
    "benefits": [
      "Lightweight serum for intense moisture",
      "Contains multiple forms of hyaluronic acid",
      "Plumps and hydrates appearance of skin",
      "Absorbs quickly without sticky feel",
      "Suitable for all skin types",
      "Fragrance-free and hypoallergenic",
      "Dermatologist tested formula",
      "Great for layering under moisturizers"
    ],
    "ingredients": "Water, Sodium Hyaluronate, Pentylene Glycol, Propanediol, Sodium Hyaluronate Crosspolymer, Panthenol, Ahnfeltia Concinna Extract, Glycerin, Trisodium Ethylenediamine Disuccinate, Citric Acid, Ethoxydiglycol, Ethylhexylglycerin, Hexylene Glycol, 1,2-Hexanediol, Phenoxyethanol, Caprylyl Glycol",
    "how_to_use": [
      "Apply a few drops to face morning and night",
      "Use after cleansing and before moisturizing",
      "Gently pat into skin",
      "Can be used around the eye area",
      "Work into damp skin for best absorption",
      "Layer with other serums as needed",
      "Wait 30 seconds before next step",
      "Daily use recommended for best results"
    ]
  },
  {
    "id": "paulas-choice-bha-exfoliant",
    "name": "Paula's Choice Skin Perfecting 2% BHA Liquid Exfoliant",
    "category": "skincare",
    "subcategory": "exfoliants",
    "price": "$38.00",
    "image_url": "https://i.pinimg.com/1200x/83/34/ca/8334caf4b8a8e0236adc68dfbcf155b4.jpg",
    "benefits": [
      "Salicylic acid (BHA) dissolves pore-clogging debris",
      "Unclogs and shrinks enlarged pores",
      "Smooths skin texture and builds collagen",
      "Reduces redness and creates radiant glow",
      "Non-abrasive, leave-on formula",
      "Dermatologist tested and fragrance-free",
      "Clinically proven results",
      "Cult-favorite for clear, smooth skin"
    ],
    "ingredients": "Water, Methylpropanediol, Butylene Glycol, Salicylic Acid, Polysorbate 20, Camellia Oleifera (Green Tea) Leaf Extract, Sodium Hydroxide, Tetrasodium EDTA",
    "how_to_use": [
      "Apply once or twice daily after cleansing and toning",
      "Lightly soak a cotton pad and apply over entire face",
      "Do not rinse",
      "Follow with moisturizer and sunscreen (daytime)",
      "Start slowly (every other day) if new to BHA",
      "Avoid direct contact with eyes and lips",
      "Store in a cool, dark place",
      "Use consistent daily for maximum benefits"
    ]
  },
  {
    "id": "the-ordinary-aha-bha-peel",
    "name": "The Ordinary AHA 30% + BHA 2% Peeling Solution",
    "category": "skincare",
    "subcategory": "exfoliants",
    "price": "$13.50",
    "image_url": "https://i.pinimg.com/1200x/22/65/df/2265df356e56d92776af495547a7e462.jpg",
    "benefits": [
      "High-concentration alpha hydroxy acids (AHA) and beta hydroxy acids (BHA)",
      "Exfoliates multiple layers of skin for brighter appearance",
      "Reduces appearance of blemishes and improves skin texture",
      "Contains Tasmanian Pepperberry to reduce irritation",
      "Weekly treatment for deep exfoliation",
      "Vegan and cruelty-free",
      "Fragrance-free and alcohol-free",
      "Professional-grade results at home"
    ],
    "ingredients": "Glycolic Acid, Water, Aloe Barbadensis Leaf Water, Sodium Hydroxide, Daucus Carota Sativa Extract, Propanediol, Cocamidopropyl Dimethylamine, Salicylic Acid, Lactic Acid, Tartaric Acid, Citric Acid, Panthenol, Sodium Hyaluronate Crosspolymer, Tasmannia Lanceolata Fruit/Leaf Extract, Glycerin, Pentylene Glycol, Xanthan gum, Polysorbate 20, Trisodium Ethylenediamine Disuccinate, Potassium Sorbate, Sodium Benzoate, Ethylhexylglycerin, 1,2-Hexanediol, Caprylyl Glycol",
    "how_to_use": [
      "Apply to clean, DRY skin",
      "Use as a mask for no more than 10 minutes",
      "Rinse thoroughly with lukewarm water",
      "Do not use more than twice per week",
      "Avoid contact with eyes and eye contour during application and rinse off",
      "Use sunscreen and limit sun exposure while using this product",
      "Not recommended for sensitive or compromised skin",
      "Patch test before full application"
    ]
  },
  {
    "id": "thayers-witch-hazel",
    "name": "Thayers Witch Hazel Facial Toner with Aloe Vera",
    "category": "skincare",
    "subcategory": "toners",
    "price": "$14.99",
    "image_url": "https://i.pinimg.com/736x/ac/bc/77/acbc77e5d0847582b8ce97bd7a7e693c.jpg",
    "benefits": [
      "Alcohol-free toner cleanses, tones, and moisturizes",
      "Contains certified organic witch hazel and aloe vera",
      "Soothes and calms irritated skin",
      "Balances pH without stripping natural oils",
      "Suitable for all skin types",
      "Cruelty-free and vegan formula",
      "Available in multiple scents (Unscented, Rose Petal, Lavender)",
      "Affordable classic skincare essential"
    ],
    "ingredients": "Water, Glycerin, Certified Organic Witch Hazel Extract Blend (Hamamelis Virginiana Extract (Witch Hazel), Aloe Barbadensis Leaf Juice (Filet of Aloe Vera)), Phenoxyethanol, Caprylyl Glycol, Ethylhexylglycerin, Citric Acid, Potassium Hydroxide",
    "how_to_use": [
      "Apply with a cotton ball or soft pad",
      "Use after cleansing and before moisturizing",
      "Apply twice daily morning and evening",
      "Can be used as a refreshing mist throughout the day",
      "Gently sweep over face and neck",
      "Allow to air dry or pat gently",
      "Safe for sensitive skin",
      "Use consistently for best skin clarity"
    ]
  },
  {
    "id": "laneige-cream-skin-refiner",
    "name": "Laneige Cream Skin Refiner",
    "category": "skincare",
    "subcategory": "toners",
    "price": "$37.00",
    "image_url": "https://i.pinimg.com/1200x/2b/50/fe/2b50fe596d08f9fc375fad40d3a08242.jpg",
    "benefits": [
      "2-in-1 hybrid toner and moisturizer",
      "Deeply hydrates like a cream with the lightness of a toner",
      "Contains White Tea Leaf Water rich in amino acids",
      "Strengthens skin's moisture barrier",
      "Absorbs instantly without greasy feel",
      "Suitable for all skin types especially dry and sensitive",
      "Fragrance-free and dermatologist tested",
      "Provides long-lasting hydration throughout the day"
    ],
    "ingredients": "Water, Glycerin, Limnanthes Alba (Meadowfoam) Seed Oil, 1,2-Hexanediol, Polyglyceryl-10 Stearate, Glyceryl Stearate Citrate, Sodium Stearoyl Glutamate, Inulin Lauryl Carbamate, Glyceryl Caprylate, Ethylhexylglycerin, Propanediol, Disodium EDTA, Camellia Sinensis Leaf Extract, Tocopherol",
    "how_to_use": [
      "Use in the morning and evening at the toner stage",
      "Dispense an appropriate amount onto palm",
      "Gently pat onto face and neck until absorbed",
      "If skin is very dry, apply another layer",
      "Can be used as a quick mask with cotton pads",
      "Works well under makeup and sunscreen",
      "Keep bottle cap tightly closed after use",
      "Store in a cool, dry place"
    ]
  },
  {
    "id": "maybelline-fit-me-foundation",
    "name": "Maybelline Fit Me Matte + Poreless Foundation",
    "category": "makeup",
    "subcategory": "foundation",
    "price": "$10.99",
    "image_url": "https://i.pinimg.com/736x/0c/f9/1e/0cf91edb1c66c8d6793eb20f5d9edc4e.jpg",
    "benefits": [
      "Natural-looking matte finish foundation",
      "Controls shine and blurs pores",
      "Affordable and available in 40+ shades",
      "Lightweight, breathable formula",
      "Matches skin tone and texture",
      "Non-comedogenic and dermatologist tested",
      "Ideal for normal to oily skin types",
      "Great buildable coverage for daily wear"
    ],
    "ingredients": "Water, Cyclohexasiloxane, Nylon-12, Isododecane, Alcohol Denat., Cyclopentasiloxane, PEG-10 Dimethicone, Cetyl PEG/PPG-10/1 Dimethicone, PEG-20, Polyglyceryl-4 Isostearate, Disteardimonium Hectorite, Phenoxyethanol, Magnesium Sulfate, Disodium Stearoyl Glutamate, HDI/Trimethylol Hexyllactone Crosspolymer, Methylparaben, Acrylates Copolymer, Tocopherol, Butylparaben, Aluminum Hydroxide, Silicic Acid",
    "how_to_use": [
      "Apply to face and blend with fingertips, brush, or sponge",
      "Focus on areas needing more coverage",
      "Start from the center of face and work outwards",
      "Allow to set before applying powder",
      "Repeat application for desired coverage",
      "Can blend for a more sheer look",
      "Works well with primers for longevity",
      "Cleanse skin before application for smoother finish"
    ]
  },
  {
    "id": "loreal-true-match-foundation",
    "name": "L'Oréal True Match Super-Blendable Foundation",
    "category": "makeup",
    "subcategory": "foundation",
    "price": "$14.00",
    "image_url": "https://i.pinimg.com/736x/98/1a/27/981a27e045c07c0ac79f1b3b4e6878c1.jpg",
    "benefits": [
      "Super-blendable formula matches skin's unique undertones",
      "Available in 40+ true-to-skin shades",
      "Hydrating formula with Pro-Vitamin B5 and Vitamin E",
      "Light to medium coverage with a natural finish",
      "Fragrance-free and non-comedogenic",
      "Formulated to match skin's texture and tone",
      "SPF 17 protection in select shades",
      "Provides a healthy-looking, radiant glow"
    ],
    "ingredients": "Water, Isododecane, Cyclopentasiloxane, Glycerin, Cyclohexasiloxane, PEG-10 Dimethicone, Methyl Methacrylate Crosspolymer, Butylene Glycol, Dimethicone, Isoeicosane, Disteardimonium Hectorite, Cetyl PEG/PPG-10/1 Dimethicone, Sodium Chloride, C9-15 Fluoroalcohol Phosphates, Polyglyceryl-4 Isostearate, Hexyl Laurate, Isostearyl Neopentanoate, Diazolidinyl Urea, Methylparaben, Propylparaben, Tocopherol, Panthenol.",
    "how_to_use": [
      "Dot foundation onto forehead, cheeks, and chin",
      "Blend outward using a makeup sponge or brush",
      "Apply more in areas that need extra coverage",
      "Layer for buildable coverage from light to medium",
      "Allow to set before applying additional makeup",
      "Can be mixed with moisturizer for a more sheer finish",
      "Works best on clean, primed skin",
      "Use daily for a flawless, natural look"
    ]
  },
  {
    "id": "estee-lauder-double-wear",
    "name": "Estée Lauder Double Wear Stay-in-Place Foundation",
    "category": "makeup",
    "subcategory": "foundation",
    "price": "$42.00",
    "image_url": "https://i.pinimg.com/1200x/c1/56/97/c156970ef669445dfec4db7d31614d26.jpg",
    "benefits": [
      "24-hour long-wear foundation with matte finish",
      "Oil-free and oil-controlling formula",
      "Available in 50+ shades with multiple undertones",
      "Full coverage that doesn't feel heavy",
      "Transfer-resistant and smudge-proof",
      "Non-comedogenic and dermatologist tested",
      "Ideal for all skin types, especially oily",
      "Perfect for long events or humid climates"
    ],
    "ingredients": "Water, Cyclopentasiloxane, Trimethylsiloxysilicate, PEG/PPG-18/18 Dimethicone, Butylene Glycol, Tribhenin, Polyglyceryl-3 Diisostearate, Magnesium Sulfate, Tocopheryl Acetate, Polymethylsilsesquioxane, Methicone, Laureth-7, Xanthan Gum, Alumina, Sodium Dehydroacetate, Phenoxyethanol, Cellulose Gum",
    "how_to_use": [
      "Apply to clean, moisturized skin",
      "Use a foundation brush or beauty sponge for best results",
      "Blend outwards from the center of the face",
      "Build coverage where needed",
      "Sets quickly, so blend in sections",
      "No setting powder required for most skin types",
      "Use oil-based cleanser for removal",
      "Work in small amounts for a natural finish"
    ]
  },
  {
    "id": "loreal-true-match",
    "name": "L'Oréal True Match Super-Blendable Foundation",
    "category": "makeup",
    "subcategory": "foundation",
    "price": "$13.99",
    "image_url": "https://i.pinimg.com/736x/98/1a/27/981a27e045c07c0ac79f1b3b4e6878c1.jpg",
    "benefits": [
      "Lightweight foundation that matches 100% of skin tones",
      "Super-blendable formula for a natural finish",
      "Available in cool, neutral, and warm undertones",
      "SPF 17 broad spectrum protection",
      "Oil-free and non-comedogenic",
      "Hydrates skin with Vitamin E and Provitamin B5",
      "Good for all skin types including sensitive",
      "Provides medium coverage that looks like skin"
    ],
    "ingredients": "Water, Isododecane, Cyclopentasiloxane, Glycerin, Cyclohexasiloxane, PEG-10 Dimethicone, Methyl Methacrylate Crosspolymer, Butylene Glycol, Dimethicone, Isoeicosane, Disteardimonium Hectorite, Cetyl PEG/PPG-10/1 Dimethicone, Sodium Chloride, C9-15 Fluoroalcohol Phosphate, Polyglyceryl-4 Isostearate, Hexyl Laurate, Isostearyl Neopentanoate, Phenoxyethanol, Caprylyl Glycol, Methylparaben, Butylparaben, Tocopherol, Panthenol",
    "how_to_use": [
      "Shake well before use",
      "Apply to clean skin with fingertips or sponge",
      "Blend outwards until seamless",
      "Apply more for increased coverage",
      "Use with a primer for smoother application",
      "Sets with a natural, skin-like finish",
      "Can be used daily without irritation",
      "Use makeup remover to cleanse at end of day"
    ]
  },
  {
    "id": "tarte-shape-tape",
    "name": "Tarte Shape Tape Full Coverage Concealer",
    "category": "makeup",
    "subcategory": "concealer",
    "price": "$32.00",
    "image_url": "https://i.pinimg.com/736x/16/53/a2/1653a2fd757b613575db588722dc3881.jpg",
    "benefits": [
      "Award-winning full coverage concealer",
      "Blurs fine lines and wrinkles for a smooth finish",
      "Long-wearing and crease-proof formula",
      "Infused with shea butter and mango seed butter",
      "Available in 30+ shades",
      "Vegan and cruelty-free",
      "Ideal for covering dark circles and blemishes",
      "Can be used for highlighting and contouring"
    ],
    "ingredients": "Water, Cyclopentasiloxane, Methyl Methacrylate Crosspolymer, Cyclohexasiloxane, Butylene Glycol, PEG-10 Dimethicone, Trimethylsiloxysilicate, Sodium Chloride, Cetyl PEG/PPG-10/1 Dimethicone, Triethyl Citrate, Disteardimonium Hectorite, Phenoxyethanol, Dimethicone/Vinyl Dimethicone Crosspolymer, Silica, Amylopectin, Aluminum Hydroxide, Triethoxycaprylylsilane, Ethylhexylglycerin, Disodium EDTA, Butyrospermum Parkii (Shea Butter), Mangifera Indica (Mango) Seed Butter",
    "how_to_use": [
      "Apply dots to under-eye area or blemishes",
      "Blend out with a concealer brush or finger",
      "For highlighting, use a shade 1-2 times lighter",
      "For contouring, use a shade 2-3 times darker",
      "Sets quickly, so blend immediately after application",
      "Use sparingly - high pigmentation",
      "Can apply over or under foundation",
      "Works well with setting powder for extra longevity"
    ]
  },
  {
    "id": "maybelline-age-rewind",
    "name": "Maybelline Instant Age Rewind Eraser Multi-Use Concealer",
    "category": "makeup",
    "subcategory": "concealer",
    "price": "$12.99",
    "image_url": "https://i.pinimg.com/1200x/ec/f5/a0/ecf5a0d85bb97e7efa59187ba15ebcf2.jpg",
    "benefits": [
      "Multi-use concealer with built-in sponge applicator",
      "Blurs dark circles and fine lines visually",
      "Contains goji berry and Haloxyl for treatment benefits",
      "Clean, natural finish that doesn't cake",
      "Available in 15+ shades",
      "Affordable and easy to apply",
      "Perfect for touch-ups throughout the day",
      "Good for mature or developing fine lines"
    ],
    "ingredients": "Water, Cyclopentasiloxane, Dimethicone, Isododecane, Glycerin, PEG-9 Polydimethylsiloxyethyl Dimethicone, Butylene Glycol, Dimethicone Crosspolymer, Nylon-12, Disteardimonium Hectorite, Cyclohexasiloxane, PEG-10 Dimethicone, Cetyl PEG/PPG-10/1 Dimethicone, Phenoxyethanol, Sodium Chloride, Polyglyceryl-4 Isostearate, Caprylyl Glycol, Disodium Stearoyl Glutamate, Ethylhexylglycerin, Methylparaben, Lycium Barbarum Fruit Extract, Chlorphenesin, Ethylparaben, Aluminum Hydroxide",
    "how_to_use": [
      "Twist collar of applicator until concealer is visible on sponge",
      "Dab under eyes or on imperfections",
      "Blend outwards with the sponge or fingertips",
      "Apply more to areas needing extra coverage",
      "Can be used for minor contouring or highlighting",
      "No additional brush needed for application",
      "Clean sponge applicator regularly",
      "Discard product after recommended period of use"
    ]
  },
  {
    "id": "maybelline-superstay-matte-ink",
    "name": "Maybelline SuperStay Matte Ink Liquid Lipstick",
    "category": "makeup",
    "subcategory": "lip products",
    "price": "$13.49",
    "image_url": "https://i.pinimg.com/736x/7a/af/61/7aaf6134926fae774420592d6e6de4ed.jpg",
    "benefits": [
      "16-hour long-wear liquid lipstick with matte finish",
      "Highly pigmented color that doesn't smudge",
      "Unique arrow applicator for precise application",
      "Available in 40+ saturated shades",
      "Transfer-proof and food-proof",
      "Dermatologist tested formula",
      "Stays vibrant throughout the day",
      "Budget-friendly long-wear lip color"
    ],
    "ingredients": "Isododecane, Dimethicone, Trimethylsiloxysilicate, Polymethylsilsesquioxane, Polypropylene, Disteardimonium Hectorite, Propylene Carbonate, Phenoxyethanol, Ethylhexylglycerin, Tocopherol",
    "how_to_use": [
      "Apply to clean, dry lips",
      "Start from the center and work outwards",
      "Use the arrow tip for precise lining",
      "Allow to dry completely (about 2 minutes) for transfer-proof finish",
      "Avoid pressing lips together until dry",
      "Reapply as needed throughout the day",
      "Use oil-based makeup remover for easy cleaning",
      "Layer with a lip balm for extra comfort"
    ]
  },
  {
    "id": "fenty-beauty-gloss-bomb",
    "name": "Fenty Beauty Gloss Bomb Universal Lip Luminizer",
    "category": "makeup",
    "subcategory": "lip products",
    "price": "$25.00",
    "image_url": "https://i.pinimg.com/736x/df/eb/fe/dfebfe6b3586fb4bdf669b5f930d48b8.jpg",
    "benefits": [
      "Ultimate high-shine lip gloss for all skin tones",
      "Non-sticky, comfortable formula",
      "Infused with shea butter to condition lips",
      "Available in multiple universally flattering shades",
      "Peach-vanilla scent is addictive",
      "Makes lips look instantly fuller and smoother",
      "Cult-favorite gloss beloved by beauty enthusiasts",
      "Vegan and cruelty-free"
    ],
    "ingredients": "Hydrogenated Polyisobutene, Hydrogenated Polydecene, Ethylene/Propylene/Styrene Copolymer, Butylene/Ethylene/Styrene Copolymer, PEG-8 Dimethicone, Butyrospermum Parkii (Shea Butter), Parfum, Tocopherol, CI 77891 (Titanium Dioxide), CI 77491 (Iron Oxides)",
    "how_to_use": [
      "Apply directly to lips with the wand applicator",
      "Wear alone for a natural high-shine look",
      "Layer over lipstick for added dimension",
      "Reapply throughout the day as desired",
      "The large wand allows for quick, one-swipe application",
      "Can be used over lip liner for a defined look",
      "Keep applicator clean for best results",
      "Store in a cool, dry place"
    ]
  },
  {
    "id": "mac-matte-lipstick",
    "name": "MAC Matte Lipstick",
    "category": "makeup",
    "subcategory": "lip products",
    "price": "$23.00",
    "image_url": "https://i.pinimg.com/1200x/87/df/e1/87dfe19e50d8fbd9e1b8af32104a371a.jpg",
    "benefits": [
      "Iconic matte lipstick with high color payoff",
      "Creamy, comfortable matte finish",
      "Available in hundreds of shades including cult classics",
      "Long-wearing and smudge-resistant",
      "Defines and shapes lips with ease",
      "Dermatologist tested formula",
      "The standard for professional-grade lip color",
      "Wide range of tones from nudes to bolds"
    ],
    "ingredients": "Octyldodecanol, Ricinus Communis (Castor) Seed Oil, Silica, Tricaprylyl Citrate, Ozokerite, Isononyl Isononanoate, Paraffin, Phenyl Trimethicone, Microcrystalline Wax, Ethylhexyl Palmitate, Vinyl Dimethicone/Methicone Silsesquioxane Crosspolymer, Caprylic/Capric Triglyceride, Copernicia Cerifera (Carnauba) Wax, Stearyl Stearoyl Stearate, Ascorbyl Palmitate, Tocopherol, Vanillin",
    "how_to_use": [
      "Apply directly to lips from the bullet",
      "For more precision, use a lip brush",
      "Line lips first for a defined look",
      "Blot with a tissue for a soft-focus effect",
      "Reapply as desired for intensity",
      "Works best on exfoliated, moisturized lips",
      "Mix shades to create custom colors",
      "Classic bullet design for easy application"
    ]
  },
  {
    "id": "loreal-lash-paradise",
    "name": "L'Oréal Voluminous Lash Paradise Mascara",
    "category": "makeup",
    "subcategory": "mascara",
    "price": "$14.99",
    "image_url": "https://i.pinimg.com/736x/fd/70/32/fd7032871d1f5b70efabf40fafffbee6.jpg",
    "benefits": [
      "Volumizing and lengthening mascara for intense look",
      "Soft wavy brush catches every lash",
      "Smooth, creamy formula leaves lashes silky",
      "No flaking, no smudging, no clumping",
      "Dermatologist and ophthalmologist tested",
      "Suitable for sensitive eyes and contact lens wearers",
      "Available in multiple colors (Blackest Black, Black, Black Brown)",
      "High-end performance at drug store price"
    ],
    "ingredients": "Water, Paraffin, Potassium Cetyl Phosphate, Copernicia Cerifera Cera, Ethylene/Acrylic Acid Copolymer, Styrene/Acrylates/Ammonium Methacrylate Copolymer, Cera Alba, Synthetic Beeswax, Bis-Diglyceryl Polyacyladipate-2, Polybutene, Cetyl Alcohol, Steareth-20, Glyceryl Dibehenate, Steareth-2, Phenoxyethanol, Silica, Hydroxyethylcellulose, Acacia Senegal Gum",
    "how_to_use": [
      "Position brush at base of lashes",
      "Wiggle brush and sweep up to tips",
      "Apply multiple coats for added volume",
      "Avoid letting mascara dry between coats",
      "Remove easily with soap and water or makeup remover",
      "Keep wand clean for best application",
      "Replace every 3 months for hygiene",
      "Comes in waterproof version for extra longevity"
    ]
  },
  {
    "id": "maybelline-sky-high",
    "name": "Maybelline Lash Sensational Sky High Mascara",
    "category": "makeup",
    "subcategory": "mascara",
    "price": "$16.99",
    "image_url": "https://i.pinimg.com/1200x/2b/3a/a2/2b3aa20be4cba883ce14c955bb775381.jpg",
    "benefits": [
      "Lengthening mascara with exclusive Flex Tower brush",
      "Reaches every lash for full volume and length",
      "Infused with bamboo extract for healthy lashes",
      "Available in multiple intensities (Blackest Black, Very Black, Brownish Black)",
      "Smudge-proof and flake-proof for all-day wear",
      "Suitable for sensitive eyes and contact lens wearers",
      "Washable and waterproof versions available",
      "Trending favorite for extreme lash length"
    ],
    "ingredients": "Water, Propylene Glycol, Styrene/Acrylates/Ammonium Methacrylate Copolymer, Polyurethane-35, Cera Alba, Synthetic Fluorphlogopite, Glyceryl Stearate, Cetyl Alcohol, PEG-200 Glyceryl Stearate, Ethylenediamine/Stearyl Dimer Dilinoleate Copolymer, Copernicia Cerifera Cera, Stearic Acid, Palmitic Acid, Ethylene/VA Copolymer, Alcohol Denat., Paraffin",
    "how_to_use": [
      "Apply to base of lashes and sweep to tips",
      "Wiggle wand to ensure every lash is coated",
      "Layer for dramatic length and volume",
      "Use the tip of the wand for lower lashes",
      "Allow to dry completely",
      "Use makeup remover for easy cleaning",
      "Replace regularly every 3 months",
      "Great for daily use or special occasions"
    ]
  },
  {
    "id": "stila-stay-all-day-eyeliner",
    "name": "Stila Stay All Day Waterproof Liquid Eye Liner",
    "category": "makeup",
    "subcategory": "eyeliner",
    "price": "$28.00",
    "image_url": "https://i.pinimg.com/736x/4d/d7/6d/4dd76dab471cde92ab4368e226db6966.jpg",
    "benefits": [
      "Waterproof liquid eyeliner with precise felt tip applicator",
      "Stay-all-day formula won't smudge or run",
      "Easy-to-use tip for both thin and bold lines",
      "Dries quickly and stays vibrant",
      "Ophthalmologist tested",
      "Available in multiple colors including classic black",
      "Perfect for creating the perfect winged look",
      "Long-wear performance trusted by professionals"
    ],
    "ingredients": "Water, Styrene/Acrylates Copolymer, Butylene Glycol, Sodium Hyaluronate, Phenoxyethanol, Ethylhexylglycerin, Sodium Dehydroacetate, CI 77266 (Carbon Black)",
    "how_to_use": [
      "Shake well before use",
      "Position tip as close to lash line as possible",
      "Draw small strokes to build a solid line",
      "Extend outwards for a winged effect",
      "Store horizontally for best ink flow",
      "Remove with oil-based makeup remover",
      "Suitable for all eye shapes",
      "Allow to dry for 30 seconds after application"
    ]
  },
  {
    "id": "nyx-epic-ink-liner",
    "name": "NYX Professional Makeup Epic Ink Liner",
    "category": "makeup",
    "subcategory": "eyeliner",
    "price": "$13.00",
    "image_url": "https://i.pinimg.com/736x/f9/b4/e3/f9b4e3c85e11ca31c3a6f732db99a57a.jpg",
    "benefits": [
      "Professional-grade liquid eyeliner with brush tip",
      "Highly pigmented, waterproof formula",
      "Creates smooth, fluid lines effortlessly",
      "Smudge-proof for 24-hour long wear",
      "Vegan and cruelty-free",
      "Precise control for any eye look",
      "Affordable alternative to high-end liners",
      "Continuous ink flow for consistent application"
    ],
    "ingredients": "Water, Styrene/Acrylates Copolymer, Propylene Glycol, PEG-40 Hydrogenated Castor Oil, Phenoxyethanol, Ethylhexylglycerin, Sodium Dehydroacetate, CI 77266 (Carbon Black)",
    "how_to_use": [
      "Shake well before each use",
      "Glide the brush tip along your lash line",
      "Press lightly for fine lines, firmer for bold lines",
      "Wing it out for a dramatic cat-eye",
      "Keep cap tightly closed to prevent drying",
      "Easy removal with waterproof makeup remover",
      "Suitable for even sensitive eyes",
      "Perfect for daily and intense makeup looks"
    ]
  },
  {
    "id": "clinique-take-the-day-off",
    "name": "Clinique Take The Day Off Cleansing Balm",
    "category": "skincare",
    "subcategory": "cleansers",
    "price": "$42.00",
    "image_url": "https://i.pinimg.com/1200x/79/40/5e/79405eb4e6093988f3f018b40f19fa59.jpg",
    "benefits": [
      "Lightweight makeup remover dissolves stubborn makeup",
      "Transforms from solid balm to silky oil",
      "Cleanses thoroughly and rinses off completely",
      "Gentle enough for whole face and eye area",
      "Non-greasy and non-drying formula",
      "Dermatologist tested and fragrance-free",
      "Suitable for all skin types",
      "Leaves skin feeling fresh and clean"
    ],
    "ingredients": "Ethylhexyl Palmitate, Carthamus Tinctorius (Safflower) Seed Oil, Caprylic/Capric Triglyceride, Sorbeth-30 Tetraoleate, Polyethylene, PEG-5 Glyceryl Triisostearate, Water, Tocopherol, Phenoxyethanol",
    "how_to_use": [
      "Scoop a small amount with dry fingertips",
      "Massage over dry skin, including eyes",
      "Rinse well with warm water",
      "Follow with regular cleanser if double cleansing",
      "Pat skin dry with a soft towel",
      "Use daily in your PM routine",
      "Safe for contact lens wearers",
      "Store in a cool, dry place"
    ]
  },
  {
    "id": "glossier-milky-jelly",
    "name": "Glossier Milky Jelly Cleanser",
    "category": "skincare",
    "subcategory": "cleansers",
    "price": "$23.00",
    "image_url": "https://i.pinimg.com/1200x/9e/a3/af/9ea3af393bf8a3af93629310de4b46f1.jpg",
    "benefits": [
      "Conditioning face wash with bouncy, jelly texture",
      "Gentle enough to use on dry skin or as makeup remover",
      "pH-balanced formula preserves skin's natural barrier",
      "Contains rose water to soothe and calm skin",
      "Ophthalmologist-tested and safe for eye area",
      "Vegan and cruelty-free",
      "Leaves skin soft, smooth, and hydrated",
      "Cleanses without stripping essential oils"
    ],
    "ingredients": "Water, Rosa Damascena Flower Water, PEG-7 Glyceryl Cocoate, Propanediol, Isohexadecane, Poloxamer 184, Acrylamide/Sodium Acryloyldimethyltaurate Copolymer, Polysorbate 80, Glycerin, Sodium Hyaluronate, Panthenol, Phenoxyethanol, Ethylhexylglycerin",
    "how_to_use": [
      "On dry skin: massage in to dissolve makeup, then rinse",
      "On wet skin: use as a daily cleanser morning or night",
      "Gently massage over face and neck",
      "Rinse with lukewarm water and pat dry",
      "No double cleansing necessary for light makeup",
      "Use as often as needed throughout the day",
      "Specifically great for sensitive or dry skin",
      "Keep out of eyes for maximum comfort"
    ]
  },
  {
    "id": "cerave-sa-cleanser",
    "name": "CeraVe SA Smoothing Cleanser",
    "category": "skincare",
    "subcategory": "cleansers",
    "price": "$16.99",
    "image_url": "https://i.pinimg.com/1200x/91/06/46/910646a94eda5a4304f50b2cf027ad52.jpg",
    "benefits": [
      "Exfoliating cleanser with salicylic acid (SA)",
      "Gently removes dirt and oil while softening skin",
      "Non-comedogenic and fragrance-free",
      "Contains 3 essential ceramides for skin health",
      "Helps improve skin texture and reduce blemishes",
      "Dermatologist recommended for rough and bumpy skin",
      "Suitable for use on face and body",
      "Cleanses effectively without disrupting skin barrier"
    ],
    "ingredients": "Water, Glycerin, Cocamidopropyl Hydroxysultaine, Sodium Lauroyl Sarcosinate, Sodium Chloride, PEG-150 Pentaerythrityl Tetrastearate, Salicylic Acid, Ceramide NP, Ceramide AP, Ceramide EOP, Phytosphingosine, Cholesterol, Sodium Lauroyl Lactylate, Methylparaben, Propylparaben, Xanthan Gum, EDTA, Sodium Hydroxide",
    "how_to_use": [
      "Apply to wet skin daily or as needed",
      "Massage gently until it lathers",
      "Rinse thoroughly with lukewarm water",
      "Use on body areas like arms and legs if needed",
      "Avoid direct contact with eyes",
      "Follow with moisturizer and sunscreen",
      "Safe for regular use on non-compromised skin",
      "Keep bottle in a dry place"
    ]
  },
  {
    "id": "aveeno-calm-restore",
    "name": "Aveeno Calm + Restore Oat Gel Moisturizer",
    "category": "skincare",
    "subcategory": "moisturizers",
    "price": "$22.99",
    "image_url": "https://i.pinimg.com/736x/2a/c6/00/2ac600bb4d4b221259e3031652e250f3.jpg",
    "benefits": [
      "Instantly soothes and replenishes skin's moisture barrier",
      "Lightweight gel formula with prebiotic oat",
      "Hypoallergenic and fragrance-free",
      "Ideal for sensitive, irritated skin",
      "24-hour hydration without greasy feel",
      "Dermatologist recommended brand",
      "Paraben-free and phthalate-free",
      "Leaves skin feeling soft and calm"
    ],
    "ingredients": "Water, Glycerin, Dimethicone, Cetyl Alcohol, Avena Sativa (Oat) Kernel Flour, Panthenol, Bisabolol, Chrysanthemum Parthenium (Feverfew) Flower/Leaf/Stem Juice, Sodium Hydroxide, Disodium EDTA, Phenoxyethanol, Ethylhexylglycerin",
    "how_to_use": [
      "Apply to clean skin morning and evening",
      "Massage gently into face and neck",
      "Can be used after serums",
      "Safe for sensitive skin daily",
      "Use as a soothing mask by applying a thicker layer",
      "Rinse with water if applying as mask",
      "Works well under sunscreen",
      "Store in a cool place"
    ]
  },
  {
    "id": "neutrogena-bright-boost",
    "name": "Neutrogena Bright Boost Gel Cream",
    "category": "skincare",
    "subcategory": "moisturizers",
    "price": "$26.49",
    "image_url": "https://i.pinimg.com/736x/bd/1a/40/bd1a40da47f34975e84fec5fda8dc3c7.jpg",
    "benefits": [
      "Visibly brightens and retextures skin",
      "Contains Neoglucosamine to boost surface cell turnover",
      "Hydrates and smooths skin for a radiant look",
      "AHA and PHA for gentle exfoliation",
      "Lightweight gel-like texture",
      "Suitable for all skin types",
      "Helps improve skin tone and clarity",
      "Oil-free and non-comedogenic"
    ],
    "ingredients": "Water, Dimethicone, Glycerin, Mandelic Acid, Gluconolactone, Sodium Hyaluronate, Squalane, Acetyl Glucosamine, PEG-10 Dimethicone, Cetyl Alcohol, Stearic Acid, Palmitic Acid, Polyacrylate Crosspolymer-6, Phenoxyethanol, Caprylyl Glycol",
    "how_to_use": [
      "Apply twice daily to cleansed face and neck",
      "Smooth evenly until fully absorbed",
      "Use as part of your daily brightening routine",
      "Sunburn Alert: This product contains AHAs",
      "Use sunscreen while using this product",
      "Follow with moisturizer if needed",
      "Can be layered with other Bright Boost products",
      "Avoid eye area"
    ]
  },
  {
    "id": "the-inkey-list-oat-cleansing-balm",
    "name": "The INKEY List Oat Cleansing Balm",
    "category": "skincare",
    "subcategory": "cleansers",
    "price": "$14.99",
    "image_url": "https://i.pinimg.com/1200x/84/c4/dd/84c4dd27a3073e392f45fd7a4e4145f6.jpg",
    "benefits": [
      "Gently dissolves makeup and sunscreen",
      "Soothing oat kernel oil and colloidal oatmeal",
      "Deeply cleanses without stripping skin",
      "Ideal for sensitive and dry skin",
      "Non-drying and non-irritating",
      "Affordable luxury cleansing experience",
      "Cruelty-free and vegan formula",
      "Fragrance-free and mineral oil-free"
    ],
    "ingredients": "Prunus Amygdalus Dulcis (Sweet Almond) Oil, Cetyl Ethylhexanoate, PEG-20 Glyceryl Triisostearate, Polyethylene, Avena Sativa (Oat) Kernel Oil, Colloidal Oatmeal, Benzyl Alcohol, Dehydroacetic Acid, Water",
    "how_to_use": [
      "Squeeze a small amount onto dry fingertips",
      "Massage onto dry skin, including eyes",
      "Add water to emulsify into a milk",
      "Rinse thoroughly with warm water",
      "Can be used as a 10-minute soothing mask",
      "Follow with a water-based cleanser if double cleansing",
      "Use daily in PM routine",
      "Store at room temperature"
    ]
  },
  {
    "id": "the-ordinary-retinol-02",
    "name": "The Ordinary Retinol 0.2% in Squalane",
    "category": "skincare",
    "subcategory": "serums",
    "price": "$11.00",
    "image_url": "https://i.pinimg.com/736x/5f/0f/bb/5f0fbbb81bde904a67f4faed22fd78c8.jpg",
    "benefits": [
      "Advanced retinol serum for anti-aging",
      "Reduces appearance of wrinkles and fine lines",
      "Improves skin texture and tone",
      "Squalane base provides hydration during treatment",
      "Gentle introduction to retinol",
      "Vegan and cruelty-free",
      "Fragrance-free and water-free formula",
      "High-value anti-aging treatment"
    ],
    "ingredients": "Squalane, Caprylic/Capric Triglyceride, Retinol, Solanum Lycopersicum (Tomato) Fruit Extract, Rosmarinus Officinalis (Rosemary) Leaf Extract, Hydroxymethoxyphenyl Decanone, BHT",
    "how_to_use": [
      "Apply a small amount to face in the PM",
      "Use after water-based serums but before heavier treatments",
      "Do not use with other retinoid treatments",
      "Avoid unprotected solar exposure",
      "Refrigerate after opening",
      "Retinoids can increase skin's sensitivity to UV radiation",
      "If irritation occurs, cease use",
      "Sun protection is essential during use"
    ]
  },
  {
    "id": "the-ordinary-vitamin-c-suspension",
    "name": "The Ordinary Vitamin C Suspension 23% + HA Spheres 2%",
    "category": "skincare",
    "subcategory": "serums",
    "price": "$10.50",
    "image_url": "https://i.pinimg.com/1200x/83/3b/21/833b21016070d5d049be760ccb6ed7e8.jpg",
    "benefits": [
      "Stable vitamin C suspension for brightening",
      "Visibly reverses signs of aging",
      "Brightens skin tone and reduces dark spots",
      "Hyaluronic acid spheres for surface smoothing",
      "Water-free and silicone-free formula",
      "Highly concentrated antioxidant protection",
      "Vegan and cruelty-free",
      "High potency results at an accessible price"
    ],
    "ingredients": "Ascorbic Acid, Squalane, Isodecyl Neopentanoate, Isononyl Isononanoate, Coconut Alkanes, Ethylene/Propylene/Styrene Copolymer, Ethylhexyl Palmitate, Silica Dimethyl Silylate, Sodium Hyaluronate, Glucomannan, Coco-Caprylate/Caprate, Butylene/Ethylene/Styrene Copolymer, Acrylates/Ethylhexyl Acrylate Crosspolymer, Trihydroxystearin, BHT",
    "how_to_use": [
      "Apply a small amount to face in the AM or PM (PM preferred)",
      "A tingling sensation can be expected after application",
      "Can be diluted with other serums/creams to reduce intensity",
      "Avoid contact with eyes",
      "Patch test before use",
      "If irritation persists, discontinue use",
      "Keep out of reach of children",
      "Storage in a cool, dark place is recommended"
    ]
  },
  {
    "id": "glow-recipe-watermelon-toner",
    "name": "Glow Recipe Watermelon Glow PHA + BHA Pore-Tight Toner",
    "category": "skincare",
    "subcategory": "toners",
    "price": "$38.00",
    "image_url": "https://i.pinimg.com/736x/6b/3c/ac/6b3cacc0521f07f5ad0bf222cfabd547.jpg",
    "benefits": [
      "Gentle PHA and BHA for pore-tightening and smoothing",
      "Hydrating watermelon extract and cactus water",
      "Bouncy, liquid texture that absorbs quickly",
      "Helps refine skin texture and minimize appearance of pores",
      "Cruelty-free and vegan formula",
      "Fragrance-free and alcohol-free",
      "Suitable for all skin types including oily and combination",
      "Leaves skin glowing and refreshed"
    ],
    "ingredients": "Water, Opuntia Ficus-Indica Stem Water, Gluconolactone, Glycerin, 1,2-Hexanediol, Salicylic Acid, Betaine Salicylate, Sodium Hyaluronate, Citrullus Lanatus (Watermelon) Fruit Extract, Panthenol, Althaea Rosea Root Extract, Citric Acid, Ethylhexylglycerin, Sodium Benzoate, Potassium Sorbate",
    "how_to_use": [
      "After cleansing, dispense into palms",
      "Gently pat onto face and neck until absorbed",
      "Use morning and night",
      "Follow with serum and moisturizer",
      "Layer for extra hydration if desired",
      "Can use with a cotton pad if preferred",
      "Store away from direct sunlight",
      "Safe for daily use"
    ]
  },
  {
    "id": "paulas-choice-enrinching-toner",
    "name": "Paula's Choice Skin Recovery Enriched Calming Toner",
    "category": "skincare",
    "subcategory": "toners",
    "price": "$26.00",
    "image_url": "https://i.pinimg.com/1200x/cb/0f/7f/cb0f7fef35356698e1d80596758aa803.jpg",
    "benefits": [
      "Milky, soothing toner for dry to very dry skin",
      "Contains antioxidant vitamins and calming plant extracts",
      "Reinforces skin's moisture barrier",
      "Reduces redness and flakiness",
      "Fragrance-free and hypoallergenic",
      "Dermatologist tested formula",
      "Leaves skin feeling soft and supple",
      "Prepares skin for optimal absorption of serums"
    ],
    "ingredients": "Water, Cyclopentasiloxane, Glycerin, Glycereth-26, Dimethiconol, Niacinamide, Sodium Hyaluronate, Oenothera Biennis (Evening Primrose) Oil, Tocopheryl Acetate, Ascorbyl Palmitate, Panthenol, Allantoin, Epilobium Angustifolium Flower/Leaf/Stem Extract, Caprylyl Glycol, Ethylhexylglycerin, Xanthan Gum, Citric Acid, Disodium EDTA, Phenoxyethanol",
    "how_to_use": [
      "Apply morning and evening after cleansing",
      "Dispense onto a cotton pad or into palms",
      "Smooth over face and neck",
      "Do not rinse",
      "Follow with serum or moisturizer",
      "Can be used as a quick skin-soothing mask",
      "Store in a cool place",
      "Safe for use on sensitive skin"
    ]
  },
  {
    "id": "fenty-beauty-eaze-drop",
    "name": "Fenty Beauty Eaze Drop Blurring Skin Tint",
    "category": "makeup",
    "subcategory": "foundation",
    "price": "$36.00",
    "image_url": "https://i.pinimg.com/736x/0f/a9/be/0fa9be9723bdef3292b718eb3b0bf758.jpg",
    "benefits": [
      "Lightweight, blurring skin tint with natural finish",
      "Provides buildable light to medium coverage",
      "Diffuses appearance of pores and imperfections",
      "Smooth, breathable formula",
      "Transfer-resistant and smudge-proof",
      "Available in 25 flexible shades",
      "Ideal for all skin types",
      "Vegan and cruelty-free"
    ],
    "ingredients": "Water, Dimethicone, Isododecane, Glycerin, PEG-10 Dimethicone, Phenyl Trimethicone, Sodium Chloride, Magnesium Sulfate, Phenoxyethanol, Ethylhexylglycerin, Tocopherol",
    "how_to_use": [
      "Shake well before use",
      "Apply with fingertips or a makeup sponge",
      "Blend outwards from the center of face",
      "Build coverage where needed",
      "Sets to a natural, blurred finish",
      "Can be layered with concealer for targeted coverage",
      "Wash hands after application",
      "Remove with makeup remover"
    ]
  },
  {
    "id": "rare-beauty-concealer",
    "name": "Rare Beauty Liquid Touch Brightening Concealer",
    "category": "makeup",
    "subcategory": "concealer",
    "price": "$23.00",
    "image_url": "https://i.pinimg.com/736x/be/a9/5d/bea95d389bdd7804143e96dd1ca6e089.jpg",
    "benefits": [
      "Lightweight, breathable concealer with brightening finish",
      "Provides medium buildable coverage",
      "Hydrates and smooths skin",
      "Infused with a botanical blend of lotus and gardenia",
      "Available in 32 shades",
      "Vegan and cruelty-free",
      "Ideal for brightening under-eyes and concealing blemishes",
      "Built-in wand for easy application"
    ],
    "ingredients": "Water, Cyclopentasiloxane, Methyl Methacrylate Crosspolymer, Glycerin, PEG-10 Dimethicone, Sodium Chloride, Phenoxyethanol, Ethylhexylglycerin, Nelumbo Nucifera Flower Extract, Gardenia Florida Fruit Extract, Tocopherol",
    "how_to_use": [
      "Dab under eyes and on imperfections",
      "Blend with fingertips or a concealer brush",
      "For extra brightening, use a shade 1-2 times lighter",
      "Can be layered over foundation",
      "Sets with a natural, skin-like glow",
      "Use sparingly - a little goes a long way",
      "Reapply throughout the day for touch-ups",
      "Remove with makeup remover"
    ]
  },
  {
    "id": "laneige-lip-sleeping-mask",
    "name": "Laneige Lip Sleeping Mask",
    "category": "makeup",
    "subcategory": "lip products",
    "price": "$28.00",
    "image_url": "https://i.pinimg.com/1200x/77/37/e5/7737e5390a46c1063557ae093df314c6.jpg",
    "benefits": [
      "Leave-on lip mask that soothes and moisturizes overnight",
      "Features Berry Mix Complex rich in vitamin C and antioxidants",
      "Moisture Wrap technology creates a protective film to lock in moisture",
      "Softens and smooths lips while you sleep",
      "Available in multiple delicious scents",
      "Can also be used as a daytime lip balm",
      "Dermatologist tested and hypoallergenic",
      "Cult-favorite for plump, healthy-looking lips"
    ],
    "ingredients": "Diisostearyl Malate, Hydrogenated Polyisobutene, Phytosteryl/Isostearyl/Cetyl/Stearyl/Behenyl Dimer Dilinoleate, Hydrogenated Poly(C6-14 Olefin), Polybutene, Microcrystalline Wax, Butyrospermum Parkii (Shea) Butter, Synthetic Wax, Euphorbia Cerifera (Candelilla) Wax, Sucrose Tetrastearate Triacetate, Ethylene/Propylene/Styrene Copolymer, Butylene/Ethylene/Styrene Copolymer, Mica (CI 77019), Titanium Dioxide (CI 77891), Dimethicone, Fragrance, Polyglyceryl-2 Diisostearate, Dehydroacetic Acid, Methicone, Copernicia Cerifera (Carnauba) Wax, Polyglyceryl-2 Triisostearate, Yellow 6 Lake (CI 15985), Red 6 (CI 15850), BHT, Water, Potassium Alginate, Glycerin, Propanediol, Alcohol, Lycium Chinense Fruit Extract, Fragaria Chiloensis (Strawberry) Fruit Extract, Rubus Idaeus (Raspberry) Fruit Extract, Sapindus Mukorossi Fruit Extract, Vaccinium Angustifolium (Blueberry) Fruit Extract, Rubus Chamaemorus Seed Extract, Coffee Arabica (Coffee) Seed Extract, Chenopodium Quinoa Seed Extract, Magnesium Sulfate, Calcium Chloride, Sodium Hyaluronate, Beta-Glucan, Manganese Sulfate, Zinc Sulfate, Ascorbyl Glucoside",
    "how_to_use": [
      "Apply generously to lips before bed using the provided spatula",
      "Leave on overnight",
      "Gently wipe off any excess in the morning",
      "Can also be used as a rich daytime lip balm",
      "Apply whenever lips feel dry or chapped",
      "For best results, use daily",
      "Store at room temperature",
      "Discard spatula if it becomes dirty"
    ]
  },
  {
    "id": "burts-bees-beeswax-lip-balm",
    "name": "Burt's Bees Beeswax Lip Balm",
    "category": "makeup",
    "subcategory": "lip products",
    "price": "$10.00",
    "image_url": "https://i.pinimg.com/1200x/7f/88/1b/7f881b50c1adc3e50033a5702e6cf814.jpg",
    "benefits": [
      "Original beeswax lip balm that naturally protects and hydrates",
      "Contains peppermint oil for a refreshing tingle",
      "Conditioning formula with vitamin E and lanolin",
      "100% natural origin",
      "Beeswax sustainably sourced",
      "Cruelty-free and formulated without parabens, phthalates, SLS, or petrolatum",
      "Budget-friendly classic lip care",
      "Leaves lips feeling soft and smooth"
    ],
    "ingredients": "Cera Alba (Beeswax, Cire D'Abeille), Cocos Nucifera (Coconut) Oil, Helianthus Annuus (Sunflower) Seed Oil, Mentha Piperita (Peppermint) Oil, Lanolin, Tocopherol, Rosmarinus Officinalis (Rosemary) Leaf Extract, Glycine Soja (Soybean) Oil, Canola Oil (Huile De Colza), Limonene",
    "how_to_use": [
      "Apply balm liberally to lips as often as needed",
      "Can be worn under or over lipstick",
      "Ideal for dry, cold, or windy conditions",
      "Focus on any particularly chapped areas",
      "Use as a base for matte lip colors",
      "Safe for children over 3 years old",
      "Store in a cool, dry place",
      "Replace after 12 months once opened"
    ]
  },
  {
    "id": "heroin-make-mascara",
    "name": "Heroine Make Long & Curl Mascara Super Waterproof",
    "category": "makeup",
    "subcategory": "mascara",
    "price": "$16.00",
    "image_url": "https://i.pinimg.com/736x/72/64/c8/7264c8fd1161eb381da92c2c9c354e1e.jpg",
    "benefits": [
      "Japanese cult-favorite mascara for extreme length and curl",
      "Super waterproof and smudge-proof formula",
      "Keeps lashes curled all day even in high humidity",
      "Contains fibers to add dramatic length",
      "Includes eyelash treatment ingredients (camellia oil, royal jelly extract, argan oil, wild rose oil)",
      "Intense black pigment for a dramatic look",
      "Resistant to tears, sweat, and sebum",
      "Professional-grade performance"
    ],
    "ingredients": "Isododecane, Trimethylsiloxysilicate, Talc, Ceresin, Disteardimonium Hectorite, Microcrystalline Wax, CI 77499, Dextrin Palmitate/Ethylhexanoate, Propylene Carbonate, Hydrogenated Polyisobutene, Nylon-66, Sucrose Tristearate, Aluminum Distearate, Glyceryl Isostearate, Trimethylsiloxysilylcarbamoyl Pullulan, Polymethylsilsesquioxane, Zinc Laurate, Silica, Camellia Japonica Seed Oil, Camellia Kissi Seed Oil, Prunus Amygdalus Dulcis (Sweet Almond) Oil, Squalane, Royal Jelly Extract, Argania Spinosa Kernel Oil, Rosa Canina Fruit Oil, Tocopherol",
    "how_to_use": [
      "Curl lashes with an eyelash curler before use",
      "Apply from the base of lashes and wiggle upwards to tips",
      "For a more dramatic effect, apply multiple coats",
      "Avoid letting mascara dry completely between coats",
      "Requires a persistent oil-based makeup remover for cleaning",
      "Keep the brush clear of excess product",
      "Replace every 3 months for hygiene",
      "Avoid direct contact with eyes"
    ]
  },
  {
    "id": "benefit-theyre-real-mascara",
    "name": "Benefit They're Real! Lengthening Mascara",
    "category": "makeup",
    "subcategory": "mascara",
    "price": "$32.00",
    "image_url": "https://i.pinimg.com/1200x/cb/29/d9/cb29d94b07a18699558838dd88e61647.jpg",
    "benefits": [
      "Lengthens, curls, volumizes, lifts, and separates lashes",
      "Long-wearing formula won't smudge or dry out",
      "Specially designed brush with staggered bristles for extreme precision",
      "Jet black color for an intense look",
      "Ophthalmologist tested",
      "Ideal for all lash types and shapes",
      "Cult-favorite for a 'false lash' effect",
      "Provides buildable volume for any occasion"
    ],
    "ingredients": "Water, Paraffin, Polybutene, Styrene/Acrylates/Ammonium Methacrylate Copolymer, Cera Alba, Bis-Diglyceryl Polyacyladipate-2, C18-36 Acid Triglyceride, Palmitic Acid, Stearic Acid, Triethanolamine, VP/Eicosene Copolymer, Acacia Senegal Gum, Hydroxyethylcellulose, Phenoxyethanol, Panthenol, Iron Oxides (CI 77499)",
    "how_to_use": [
      "Wiggle the brush from the base of lashes to the tips",
      "Use the custom-domed tip for outer corner and lower lashes",
      "Apply multiple coats for added drama and volume",
      "Do not let dry between applications",
      "Remove with soap and water or makeup remover",
      "Replace every 3 months for safety",
      "Avoid pumping the wand in the tube",
      "Safe for sensitive eyes and contact lens wearers"
    ]
  },
  {
    "id": "stila-huge-mascara",
    "name": "Stila HUGE Extreme Lash Mascara",
    "category": "makeup",
    "subcategory": "mascara",
    "price": "$28.00",
    "image_url": "https://i.pinimg.com/1200x/5a/24/2d/5a242df3ce004f79489a103656684ea1.jpg",
    "benefits": [
      "Extreme volumizing mascara for bigger, bolder lashes",
      "Clinically proven results for volume and length",
      "Intense black pigment stay vibrant all day",
      "Soft, flexible brushes for comfortable application",
      "Ophthalmologist tested and safe for sensitive eyes",
      "Long-wear formula won't flake or smudge",
      "Cruelty-free and vegan-friendly options",
      "Ideal for those wanting dramatic, high-impact lashes"
    ],
    "ingredients": "Water, Paraffin, Potassium Cetyl Phosphate, Copernicia Cerifera Cera, Ethylene/Acrylic Acid Copolymer, Styrene/Acrylates/Ammonium Methacrylate Copolymer, Cera Alba, Synthetic Beeswax, Bis-Diglyceryl Polyacyladipate-2, Polybutene, Cetyl Alcohol, Steareth-20, Glyceryl Dibehenate, Steareth-2, Phenoxyethanol, Silica, Hydroxyethylcellulose, Iron Oxides (CI 77499)",
    "how_to_use": [
      "Place brush at base of lashes and wiggle upwards to tips",
      "Apply several coats for extreme volume and length",
      "Concentrate on outer lashes for a winged effect",
      "Avoid letting dry between coats",
      "Remove easily with warm water and cleanser",
      "Keep wand clean for best results",
      "Replace regularly for lash safety",
      "Works well with lash primers"
    ]
  },
  {
    "id": "kat-von-d-tattoo-liner",
    "name": "KVD Beauty Tattoo Liner Waterfront Liquid Eyeliner",
    "category": "makeup",
    "subcategory": "eyeliner",
    "price": "$29.00",
    "image_url": "https://i.pinimg.com/736x/02/9f/d9/029fd961854cbc2d485b55c3e525f223.jpg",
    "benefits": [
      "Award-winning waterproof liquid eyeliner with precise brush tip",
      "Long-wearing formula won't fade or smudge",
      "High-pigment black color for intense eye looks",
      "Ultra-fine tip for precise control and wings",
      "Vegan and cruelty-free",
      "Built for professional accuracy and durability",
      "Ideal for all eye shapes and skill levels",
      "Classic beauty icon for over a decade"
    ],
    "ingredients": "Water, Ammonium Styrene/Acrylates Copolymer, Butylene Glycol, Alcohol Denat., Beheneth-30, Phenoxyethanol, Acrylates/Octylacrylamide Copolymer, Methylparaben, Polyglyceryl-3 Disiloxane Dimethicone, Propylparaben, Sodium Hydroxide, Disodium EDTA, CI 77266 (Carbon Black)",
    "how_to_use": [
      "Shake well before each use to activate ink flow",
      "Glide the brush tip along your upper lash line",
      "Apply light pressure for a thin line and more for bold wings",
      "Store tip-down horizontally for consistent ink quality",
      "Remove with oil-based makeup remover at end of day",
      "Safe for sensitive eyes and contact lens wearers",
      "Dries quickly for immediate results",
      "Avoid direct contact with eyes"
    ]
  },
  {
    "id": "mac-technakohl-liner",
    "name": "MAC Technakohl Liner",
    "category": "makeup",
    "subcategory": "eyeliner",
    "price": "$23.00",
    "image_url": "https://i.pinimg.com/736x/f0/6a/64/f06a6474cf8d93d97186540af8c5f2a4.jpg",
    "benefits": [
      "Retractable eye liner with creamy, smooth formula",
      "Long-wearing and smudge-resistant",
      "Dries quickly to a matte, solid finish",
      "Multiple shades available for versatile looks",
      "Safe for use on waterlines",
      "Ophthalmologist tested",
      "Easy application without needing a sharpener",
      "Professional favorite for both subtle and bold liner"
    ],
    "ingredients": "Isododecane, Synthetic Wax, Hydrogenated Polydicyclopentadiene, PEG/PPG-18/18 Dimethicone, Polybutene, Glyceryl Tribehenate/Isostearate/Eicosandioate, Nylon-12, Polyethylene, Perfluorononyl Dimethicone, Polyglyceryl-4 Diisostearate/Polyhydroxystearate/Sebacate, Synthetic Beeswax, Pentaerythrityl Tetra-Di-T-Butyl Hydroxyhydrocinnamate, Copernicia Cerifera (Carnauba) Wax, Iron Oxides (CI 77491, CI 77492, CI 77499)",
    "how_to_use": [
      "Twist bottom to advance liner tip",
      "Apply directly to lash lines or waterlines",
      "Smudge with a brush or finger for a smoky look immediately after application",
      "Allow to set for 30 seconds for long-wear finish",
      "Remove with eye makeup remover",
      "Keep cap tightly closed after use",
      "Replace cap with a 'click' to ensure it's sealed",
      "Avoid over-advancing the tip to prevent breakage"
    ]
  },
  {
    "id": "anastasia-brow-wiz",
    "name": "Anastasia Beverly Hills Brow Wiz",
    "category": "makeup",
    "subcategory": "eyeliner",
    "price": "$29.00",
    "image_url": "https://i.pinimg.com/1200x/b9/0d/e2/b90de2deacd7e3ccf9ec8ad7a63f042f.jpg",
    "benefits": [
      "Ultra-slim retractable brow pencil for professional results",
      "Creates natural-looking hair-like strokes",
      "Long-wearing, smudge-proof formula",
      "Available in multiple shades to match all hair colors",
      "Includes a built-in spoolie for blending",
      "Cult-favorite for perfect, defined brows",
      "Ideal for sparse brows or adding detail",
      "High-precision tip for ultimate control"
    ],
    "ingredients": "Hydrogenated Soybean Oil, Hydrogenated Coco-Glycerides, Hydrogenated Vegetable Oil, Zinc Stearate, Copernicia Cerifera (Carnauba) Wax, Stearic Acid, Polyglyceryl-2 Triisostearate, Ascorbyl Palmitate, Tocopherol, Caprylyl Glycol, Phenoxyethanol, Hexylene Glycol, [+/-: Mica, CI 77891 (Titanium Dioxide), CI 77491, CI 77492, CI 77499 (Iron Oxides)]",
    "how_to_use": [
      "Outline the brow shape using light strokes",
      "Fill in sparse areas with short, hair-like strokes",
      "Blend thoroughly with the spoolie brush for a natural finish",
      "Focus on the front of the brow for a softer look",
      "Build intensity for more dramatic brows",
      "No sharpening required - just twist up",
      "Store in a cool, dry place",
      "Remove with regular makeup remover"
    ]
  }
];

